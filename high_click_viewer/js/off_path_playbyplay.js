...
function frameMatchesChangesOnly(frame, changesOnly) {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (before === after) continue; // cell didn't change in this action
      const required = changesOnly[r]?.[c] ?? null;
      if (after !== required) return false; // change doesn't match a required change
    }
  }
  return true;
}
...
function renderTimeline(steps, activeIndex, timelineEl) {
  ...
  if (!steps.length) {
    const empty = document.createElement("p");
    empty.className = "playbyplay-empty";
    empty.textContent = "No steps recorded for this participant.";
...
(async function initOffPathPlayByPlayPage() {
  ...
  function renderTaskCard(task, summaryData) {
    ...
    wrong.textContent = `${summaryData.wrongStepTotal} steps off the required path${summaryData.wrongStepTotal === 1 ? "" : "s"}`;
  ...
  function buildPlayer(task, payload) {
    ...
    badges.appendChild(createChip(`${summaryData.wrongStepTotal} steps off path${summaryData.wrongStepTotal === 1 ? "" : "s"}`, "task-badge"));

    const subtitle = document.createElement("p");
    subtitle.className = "subtitle";
    subtitle.textContent = "Use the controls below to play through the participant's steps and see how they deviate from the required changes path.";
    ...
    const solvedPane = document.createElement("div");
    const solvedTitle = document.createElement("h3");
    solvedTitle.textContent = "Solved output";
    const solvedGridEl = document.createElement("div");
    solvedGridEl.className = "arc-grid";
    solvedPane.appendChild(solvedTitle);
    solvedPane.appendChild(solvedGridEl);

    const outputPane = document.createElement("div");
    const outputTitle = document.createElement("h3");
    outputTitle.textContent = "Participant frame";
    ...
    changesTitle.textContent = "Required changes";
    const changesGridEl = document.createElement("div");
    changesGridEl.className = "arc-grid";
    changesPane.appendChild(changesTitle);
    changesPane.appendChild(changesGridEl);
    ...
    let activeSteps = [];
    let changesOnlyGrid = buildChangesOnlyGrid(testInputGrid, testOutputGrid);
    ...
    function renderFrame() {
      ...
      if (!frame) {
        actionInfoEl.textContent = "No steps available.";
        currentStepPanel.innerHTML = "";
        currentStepPanel.className = "playbyplay-step is-active";
        currentStepPanel.textContent = "No step selected.";
    ...
    function loadParticipant(index) {
      ...
      changesOnlyGrid = buildChangesOnlyGrid(testInputGrid, testOutputGrid);

      participantHeading.textContent = record ? getPathParticipantId(record) : "No participant";
      participantMeta.textContent = record
        ? `Attempt ${record.attempt_number} | ${record.wrong_step_count} steps off path${record.wrong_step_count === 1 ? "" : "s"}`
        : "";
      ...
      if (record && activeSteps.length > 0) {
        ...
        participantChipRow.appendChild(createChip("No steps off path", "participant-chip"));
      }
    ...
    if (visiblePaths.length === 0) {
      ...
      visiblePaths.forEach((path, idx) => {
        ...
        cardHeader.appendChild(createChip(`${path.wrong_step_count} steps off${path.wrong_step_count === 1 ? "" : "s"}`, "task-badge"));
  ...
  function renderGrid() {
    ...
    tasks.forEach((task) => {
      ...
      if (matchesSet && matchesSearch && summaryData.paths.length > 0) {
        visible.push({ task, summaryData });
      }
    });

    visible.sort((a, b) => b.summaryData.wrongStepTotal - a.summaryData.wrongStepTotal);

    visible.forEach(({ task, summaryData }) => {
      const key = taskKey(task.task_type, task.task_name);
  ...
  try {
    ...
    statusEl.textContent = `Loaded ${tasks.length} tasks with changes-path play-by-play summaries.`;
    ...
    statusEl.textContent = `Error loading changes-path play-by-play: ${err.message}`;
